...
export function Header({
  ...
  const { user, getUserDisplayName, isAuthenticated, logout, loading } =
    useAuth();
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between pr-4 pl-4 sm:pr-6">
        <div className="flex items-center gap-3">
          ...
          <a
            ...
            className="flex items-center gap-1.5 text-white/80 hover:text-white transition-colors text-sm hidden sm:flex border border-white/20 rounded-lg px-2 py-1 justify-center"
          >
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            className="bg-white text-[#4e1cb3] hover:bg-gray-100 h-8 px-4 text-sm font-medium"
            asChild
          >
            <Link
              href={!loading && isAuthenticated ? "/submit" : "/signin"}
              prefetch
            >
              Submit
            </Link>
